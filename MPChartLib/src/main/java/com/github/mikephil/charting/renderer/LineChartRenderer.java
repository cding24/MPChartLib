...
public class LineChartRenderer extends LineRadarRenderer {
    ...
    public LineChartRenderer(Context context, LineDataProvider chart, ChartAnimator animator,
    ...
    protected void drawHorizontalBezier(ILineDataSet dataSet) {
        ...
        if (mXBounds.range >= 1) {
            ...
            for (int j = mXBounds.min + 1; j <= mXBounds.range + mXBounds.min; j++) {
                ...
                cubicPath.cubicTo(cpx, prev.getY() * phaseY, cpx, cur.getY() * phaseY, cur.getX(),
                        cur.getY() * phaseY);
            }
        }
    ...
    public void drawValues(Canvas c) {
        if (isDrawingValuesAllowed(mChart)) {
            ...
            for (int i = 0; i < dataSets.size(); i++) {
                ...
                float[] positions = trans.generateTransformedValuesLine(dataSet, mAnimator.getPhaseX(),
                ...
                for (int j = 0; j < positions.length; j += 2) {
                    ...
                    //==========added by linghu for 显示值的气泡是向上还是向下======================
                    float minYValue = -1;
                    float maxYValue = -1;
                    for (int t = 0; t < dataSets.size(); t++) {
                        ILineDataSet dataSetItem = dataSets.get(t);
                        Entry tmpEntry = dataSetItem.getEntryForIndex(j / 2 + mXBounds.min);
                        if(tmpEntry != null){
                            if(t == 0){
                                minYValue = maxYValue = tmpEntry.getY();
                            }else{
                                if(maxYValue < tmpEntry.getY()){
                                    maxYValue = tmpEntry.getY();
                                }
                                if(minYValue > tmpEntry.getY()){
                                    minYValue = tmpEntry.getY();
                                }
                            }
                        }
                    }
//                    Log.d("Linghu", "=============="+minYValue+"maxYValue="+maxYValue);
                    if(minYValue < maxYValue){
                        if(entry.getY() >= maxYValue){
                            setValueBgStyle(1, dataSet.getBgColor(), context);
                        }else if(entry.getY() <= minYValue){
                            setValueBgStyle(2, dataSet.getBgColor(), context);
                        }else{
                            setValueBgStyle(0, dataSet.getBgColor(), context);
                        }
                    }else if(minYValue == maxYValue){
                        setValueBgStyle(dataSet.getBgStyle(), dataSet.getBgColor(), context);
                    }else{
                        setValueBgStyle(0, dataSet.getBgColor(), context);
                    }
                    //=========end added by linghu =================================================
    ...
    protected void drawCircles(Canvas c) {
        ...
        for (int i = 0; i < dataSets.size(); i++) {
            ...
            boolean drawCircleHole = dataSet.isDrawCircleHoleEnabled() && circleHoleRadius < circleRadius
