...
public class Legend extends ComponentBase {
    ...
    public Legend(int[] colors, String[] labels) {
        ...
        for (int i = 0; i < Math.min(colors.length, labels.length); i++) {
            ...
            }else if (entry.formColor == ColorTemplate.COLOR_NONE || entry.formColor == 0) {
    ...
    public int[] getColors() {
        ...
        for (int i = 0; i < mEntries.length; i++) {
            old[i] = mEntries[i].form == LegendForm.NONE ? ColorTemplate.COLOR_SKIP :
                    (mEntries[i].form == LegendForm.EMPTY ? ColorTemplate.COLOR_NONE : mEntries[i].formColor);
        }
        return old;
    }
    ...
    public LegendPosition getPosition() {
        if (mOrientation == LegendOrientation.VERTICAL && mHorizontalAlignment == LegendHorizontalAlignment.CENTER
            ...
            if (mVerticalAlignment == LegendVerticalAlignment.TOP)
                return mHorizontalAlignment == LegendHorizontalAlignment.LEFT
                        ...
                        ? LegendPosition.ABOVE_CHART_RIGHT : LegendPosition.ABOVE_CHART_CENTER);
            else
                return mHorizontalAlignment == LegendHorizontalAlignment.LEFT
                        ? LegendPosition.BELOW_CHART_LEFT
                        : (mHorizontalAlignment == LegendHorizontalAlignment.RIGHT
                        ? LegendPosition.BELOW_CHART_RIGHT : LegendPosition.BELOW_CHART_CENTER);
        } else {
            if (mHorizontalAlignment == LegendHorizontalAlignment.LEFT)
                return mVerticalAlignment == LegendVerticalAlignment.TOP && mDrawInside
                        ...
                        : (mVerticalAlignment == LegendVerticalAlignment.CENTER
                        ? LegendPosition.LEFT_OF_CHART_CENTER : LegendPosition.LEFT_OF_CHART);
            else
                return mVerticalAlignment == LegendVerticalAlignment.TOP && mDrawInside
                        ? LegendPosition.RIGHT_OF_CHART_INSIDE
                        : (mVerticalAlignment == LegendVerticalAlignment.CENTER
                        ? LegendPosition.RIGHT_OF_CHART_CENTER : LegendPosition.RIGHT_OF_CHART);
        }
    }
        ...
        switch (newValue) {
            ...
            case LEFT_OF_CHART_CENTER:
                ...
                mVerticalAlignment = newValue == LegendPosition.LEFT_OF_CHART_CENTER
                        ? LegendVerticalAlignment.CENTER : LegendVerticalAlignment.TOP;
            ...
            case ABOVE_CHART_RIGHT:
                mHorizontalAlignment = newValue == LegendPosition.ABOVE_CHART_LEFT
                        ...
                        ? LegendHorizontalAlignment.RIGHT : LegendHorizontalAlignment.CENTER);
        ...
        switch (mOrientation) {
            ...
            case HORIZONTAL: {
                ...
                mNeededHeight = labelLineHeight * (float) (mCalculatedLineSizes.size())
                        + labelLineSpacing * (float) (mCalculatedLineSizes.size() == 0
                                ? 0 : (mCalculatedLineSizes.size() - 1));
                break;
            }
        }
